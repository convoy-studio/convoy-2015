...
export default class ProjectsMenu extends BaseComponent {
	...
	open() {
		if(this.isOpened) {
			...
			this.isOpened = true
			this.addEvents()
			dom(this.background).addClass('active')
		}
	}
	close() {
		this.isOpened = false
		...
		this.removeEvents()
		dom(this.background).removeClass('active')
	}
	addEvents() {
		dom(window).on("mousemove", this.onMouseMove)
	}
	...
	resizeProjects() {
		...
		for (var i = 0; i < projects.length; i++) {
			...
			this.tl.fromTo(project.parent, 1, { y:blockSize[1]+100 }, { y:0, force3D:true, ease:Expo.easeInOut }, 0.05*i)
