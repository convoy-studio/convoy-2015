...
class Router {
	init() {
		...
		// Check if is a URL like //host/works/camper and transform it like //host/#!/works/camper
		if(URI.length > 2) {
			var url = '//' + Host + '/#!' + URI
			location.href = url
		}
	...
	_setupCrossroads() {
		...
		var validSects = (AppStore.Detector.type == AppConstants.PHONE) ? ['works'] : ['landing', 'works']
		homeSection.rules = {
